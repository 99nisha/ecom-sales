...
```{r}
df_age <- plot_categorical_features(df, age_group, mean_total_thresh = 200, mean_profit_thresh = 30)
df_age
```
...
```{r}
df_delivery_method <- plot_categorical_features(df, delivery_method, mean_total_thresh = 200, mean_profit_thresh = 30)
df_delivery_method
```
...
```{r}
ggplot(df, aes(x = total_amount, y = price, colour = factor(quantity))) +
...
ggplot(df, aes(x = profit_margin, y = price, colour = factor(quantity))) +
  geom_point(alpha = 0.5, size = 1) +
  geom_abline(slope = 1, intercept = 0, colour = my_theme$blue, linetype = "dashed") +
  labs(
...
Clustered. Shows Non-Linear, Multiplicative relationships. Mainly driven by Quantity. 
Only Assumption: Possibly driven by Discounts, Shipping Costs and Category Margins.

**Shipping Cost vs Total Amount and Profit Margin**

```{r}
# Fit linear model with log(total_amount)
model_total <- lm(shipping_cost ~ log(total_amount), data = df)
summary(model_total)

# Plot with fitted log curve
ggplot(df, aes(x = total_amount, y = shipping_cost)) +
  geom_point(alpha = 0.5, size = 1, colour = "steelblue") +
  geom_smooth(method = "lm", formula = y ~ log(x), colour = "red", se = TRUE) +
  labs(
    title = "Shipping Cost vs Total Amount (Log Model)",
    x = "Total Amount",
    y = "Shipping Cost"
  ) +
  theme_minimal()
...
# Non parametric
ggplot(df, aes(x = profit_margin, y = shipping_cost)) +
  geom_point(alpha = 0.5, size = 1, colour = "steelblue") +
  labs(
    title = "Shipping Cost vs Profit Margin (with y = x reference line)",
    x = "Profit Margin",
    y = "Shipping Cost"
  ) +
  theme_minimal()
